        ...
        const emailResponse = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            headers: payload.unsubscribe_token
              ? { 'List-Unsubscribe': `<https://fenixconvertpdf.com.br/unsubscribe?token=${payload.unsubscribe_token}>` }
              : undefined,
          }),
        })
        if (!emailResponse.ok) {
          const errorText = await emailResponse.text()
          const error = new Error(`Resend API ${emailResponse.status}: ${errorText.slice(0, 500)}`)
          Object.assign(error, { status: emailResponse.status })
          throw error
        }
